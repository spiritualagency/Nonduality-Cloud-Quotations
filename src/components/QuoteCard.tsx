...
import { RefreshCw, Share2, Download, Code, Facebook, Copy, Instagram, Settings } from 'lucide-react';
...
export default function QuoteCard() {
  ...
  const fetchQuoteAndBg = async (isRefresh = false) => {
    ...
    try {
      ...
      // Fetch background image based on category
      const bgRes = await fetch(`/api/background?category=${encodeURIComponent(quoteData.category)}`);
  ...
  const fetchSpecificQuote = async (id: string) => {
    setLoading(true);
    ...
    try {
      ...
      setQuote(quote);
      setBgImage(quote.imageUrl || (await (await fetch(`/api/background?category=${encodeURIComponent(quote.category)}`)).json()).url);
  ...
  const shareUrl = quote ? `${window.location.origin}/quote/${quote.id}` : window.location.href;
      ...
      >
        ...
        <AnimatePresence>
          {showShareMenu && (
            ...
            >
              <div 
                className="bg-ivory text-charcoal p-8 rounded-3xl max-w-sm w-full"
                onClick={e => e.stopPropagation()}
              >
                <h3 className="font-serif text-2xl mb-6 text-center">Share this quote</h3>
                
                <div className="flex flex-col gap-4">
                  <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-4 bg-black/5 rounded-xl hover:bg-black/10 transition-colors">
                    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                    <span>Share on X</span>
                  </a>
                  <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-4 bg-black/5 rounded-xl hover:bg-black/10 transition-colors">
                    <Facebook size={24} />
                    <span>Share on Facebook</span>
                  </a>
                  <a href={`https://wa.me/?text=${encodeURIComponent(shareText + ' ' + shareUrl)}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-4 bg-black/5 rounded-xl hover:bg-black/10 transition-colors">
                  ...
                  <button onClick={() => { alert('To share on Instagram, please download the image and upload it manually.'); handleDownload(); }} className="flex items-center gap-3 p-4 bg-black/5 rounded-xl hover:bg-black/10 transition-colors" title="Instagram">
                    <Instagram size={24} />
                    <span>Share on Instagram</span>
                  </button>
                  <button onClick={handleCopy} className="flex items-center gap-3 p-4 bg-black/5 rounded-xl hover:bg-black/10 transition-colors">
                    <Copy size={24} />
                    <span>Copy Link</span>
                  </button>
                </div>
